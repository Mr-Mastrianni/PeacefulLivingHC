...
const ContactSection: React.FC = () => {
  ...
  // Handle local success state properly since state.succeeded persists
  useEffect(() => {
    if (state.succeeded) {
      setShowSuccess(true);
    }
        ...
        <div className="w-screen h-full flex flex-col justify-center px-8 sm:px-16 lg:px-24 bg-white relative">
          ...
          <div className="max-w-3xl space-y-12 contact-panel-content">
            ...
            <p className="text-xl text-slate-600 leading-relaxed max-w-xl">
              If you need more information or have questions, our team is ready to help.
            </p>

            <div className="mt-6">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Primary Contact</p>
              <p className="text-2xl font-serif text-slate-800">{CONTACT_PERSON}</p>
            </div>
              ...
              <div className="lg:col-span-3">
                {showSuccess ? (
                  <div className="text-center py-12">
                    ...
                    <button onClick={handleReset} className="mt-6 text-purple-600 font-bold hover:underline">Send another</button>
                  </div>
                ) : (
                  <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <input required name="name" type="text" placeholder="Full Name" className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-purple-500/10 focus:border-purple-500 outline-none transition-all placeholder:text-slate-400" />
                    <ValidationError prefix="Name" field="name" errors={state.errors} className="text-red-500 text-xs sm:col-span-2" />

                    <input required name="email" type="email" placeholder="Email" className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-purple-500/10 focus:border-purple-500 outline-none transition-all placeholder:text-slate-400" />
                    <ValidationError prefix="Email" field="email" errors={state.errors} className="text-red-500 text-xs sm:col-span-2" />

                    <input required name="message" type="hidden" value="Care Plan Request" /> {/* Hidden context field */}

                    <input required name="phone" type="tel" placeholder="Phone" className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-purple-500/10 focus:border-purple-500 outline-none transition-all sm:col-span-2 placeholder:text-slate-400" />
                    <ValidationError prefix="Phone" field="phone" errors={state.errors} className="text-red-500 text-xs sm:col-span-2" />

                    <textarea required name="details" placeholder="How can we help?" rows={3} className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-purple-500/10 focus:border-purple-500 outline-none transition-all sm:col-span-2 resize-none placeholder:text-slate-400" />
                    <ValidationError prefix="Message" field="details" errors={state.errors} className="text-red-500 text-xs sm:col-span-2" />

                    <button type="submit" disabled={state.submitting || !FORMSPREE_ID} className="sm:col-span-2 py-4 gradient-accent text-white font-bold rounded-xl shadow-lg hover:shadow-xl hover:scale-[1.01] active:scale-95 transition-all disabled:opacity-50">
                      {state.submitting ? 'Sending...' : 'Submit Request'}
                    </button>
                    {!FORMSPREE_ID && (
                      <p className="text-xs text-red-400 font-bold sm:col-span-2 text-center mt-2">Formspree ID missing in constants.tsx</p>
                    )}
                  </form>
                )}
              </div>
